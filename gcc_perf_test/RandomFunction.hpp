...
class RandomFunction
{
  ...
  bool vectorize;
public:
  RandomFunction(const int count, const int depth, const bool vectorize)
  {
  ...
  void Eval(const int w, const int h, const float* x, const float* y, 
    float* result, const bool vectorize) const
  {
    if (vectorize)
...
private:
  void EvalVector(const int w, const int h, const float* x, 
    const float* y, float* result) const
  {
    int count = w * h;
  ...
  void EvalVector(const int* count, const float* x, const float* y, float* result) const
  {
    float *xx = new float[*count];
    float *yy = new float[*count];
    switch (type)
  ...
  float Eval(const vector<float>& params) const
  {
    float result = 0;
    switch (type)
    {
